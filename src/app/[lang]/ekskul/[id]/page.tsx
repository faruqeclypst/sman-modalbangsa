...
export async function generateMetadata({
  ...
  const post = await getCPTBySlug("ekskul", id);
  ...
  return {
    title,
    description,
    alternates: { canonical: `/${lang}/ekskul/${post.slug}` },
    openGraph: {
      title,
      description,
      type: "article",
...
export default async function EkskulDetailPage({
  ...
  return (
    <CPTDetail
      ...
      locale={lang}
      dict={dict}
      typeLabel={dict.cpt.ekskul.label}
      backHref={`/${lang}/ekskul`}
      backLabel={dict.cpt.ekskul.title}
    />
  );
}
